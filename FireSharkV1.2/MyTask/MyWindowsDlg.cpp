...
void MyWindowsDlg::PrintWindowsList()
{
	...
	if (!nOn)
	{
		...
		ImageList_SetBkColor(nHiml, list_windows_.GetBkColor());//设置图标列表底色
		list_windows_.SendMessage(LVM_SETIMAGELIST, 1, (LPARAM)nHiml);//设置超级列表显示图标
	}
	...
	for (DWORD i = 0; i < nListNum; i++)
	{
		...
		DWORD_PTR nPid;
		HANDLE nHandle;
		nFileRoute = list_windows_.GetItemText(i, 0);
		nPid = list_windows_.GetItemData(i);
		...
		if (!SHGetFileInfo(nFileRoute, nFileAttributes, &nPsfi, sizeof(SHFILEINFO), SHGFI_ICON)) {
			SHGetFileInfo(nFileRoute, nFileAttributes, &nPsfi, 
