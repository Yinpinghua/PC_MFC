...
CString GetPathEx(CString nPath)
{
	...
	nIndex = nPath.Find('\"');
	if (nIndex == 0) {
		nPath = nPath.Right(nPath.GetLength() - 1);
	}
...
void ClearThreadList(THREADLIST &nThreadList)
{
	...
	nThreadList.th32ThreadID = nullptr;
	delete[]nThreadList.th32OwnerProcessID;
	nThreadList.th32OwnerProcessID = nullptr;
	delete[]nThreadList.tpBasePri;
	nThreadList.tpBasePri = nullptr;
	delete[]nThreadList.szExePath;
	nThreadList.szExePath = nullptr;
	delete[]nThreadList.szThreadState;
	nThreadList.szThreadState = nullptr;
}
...
void ClearHeapEntyr(HEAPENTYR &nHeapEntyr)
{
	...
	nHeapEntyr.dwAddress = nullptr;
	delete[]nHeapEntyr.dwBlockSize;
	nHeapEntyr.dwBlockSize = nullptr;
}
...
DWORD IpToInt(CString nIp)
{
	nIp = "";
	DWORD temp_ip = 0;
	inet_pton(AF_INET, CStringA(nIp), &temp_ip);
	return temp_ip;
}
...
DWORD GetProcessThreadId(DWORD pid)
{
	...
	nThread.dwSize = sizeof(THREADENTRY32);
	HANDLE nSnapShot = CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, pid);

	BOOL ret = Thread32First(nSnapShot, &nThread);
	while (ret){
		if (nThread.th32OwnerProcessID == pid){
			CloseHandle(nSnapShot);
			return nThread.th32ThreadID;
		}
		ret = Thread32Next(nSnapShot, &nThread);
	}
